...
static unsigned char font[] = {
    ...
    0x24, 0x84, 0x20,  0x0e, 0x0e, 0x00,  0x84, 0x24, 0x80,  0xe2, 0x40, 0x40, // < = > ?
    ...
    0x88, 0x88, 0xe0,  0xae, 0xea, 0xa0,  0xae, 0xee, 0xa0,  0x4a, 0xaa, 0x40, // L M N O
    0xca, 0xac, 0x80,  0x4a, 0xae, 0x60,  0xca, 0xec, 0xa0,  0x68, 0x42, 0xc0, // P Q R S
...
static void render_string(char *str, int *buffer, int x, int y, int width) {
    int curx = x;
    while(*str) { // null terminated string
        if(*str == '\n') {
            curx = x;
            y += 6;
        } else {
            render_character(*str, buffer, curx, y, width);
            curx += 4;
        }
        str++;
    }
